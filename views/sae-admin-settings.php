...
if ( ! defined( 'ABSPATH' ) ) {
    ...
    if($action === 'change_period' &&  isset( $_GET['tp'] )) {
        update_option( 'sender_automated_emails_cart_period', strtolower(sanitize_text_field($_GET['tp'])) );
    ...
    if($action === 'authenticate' && isset( $_GET['response_key'] )) {
        $key = sanitize_text_field($_GET['response_key']);
        $sender_helper->authenticate($key);
    }
